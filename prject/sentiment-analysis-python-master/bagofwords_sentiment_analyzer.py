...
for pos_word in open(POS_WORDS_FILE, 'r',encoding = "ISO-8859-1").readlines()[35:]:
    pos_words.append(({pos_word.rstrip(): True}, 'positive'))

for neg_word in open(NEG_WORDS_FILE, 'r', encoding = "ISO-8859-1").readlines()[35:]:
...
def run_sentiment_analysis_on_rt():
    rt_positive_reviewers = open(POSITIVE_REVIEWS_FILE, 'r',encoding = "ISO-8859-1")
    ...
    rt_negative_reviews = open(NEGATIVE_REVIEWS_FILE, 'r', encoding = "ISO-8859-1")
    ...
    print ('pos precision: %.2f' % nltk.precision(expected_pos_set['positive'], actual_pos_set['positive']))
    print ('pos recall: %.2f' % nltk.recall(expected_pos_set['positive'], actual_pos_set['positive']))
    print ('neg precision: %.2f' % nltk.precision(expected_neg_set['negative'], actual_neg_set['negative']))
    print ('neg recall: %.2f' % nltk.recall(expected_neg_set['negative'], actual_neg_set['negative']))

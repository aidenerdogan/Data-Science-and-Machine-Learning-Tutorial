...
def get_sentiment(words):
	...
	for word in words:
		if word in data_set[1]:
			p_word_positive_temp =((data_set[1][word]/sum(data_set[1].values()))*(sum(data_set[1].values())/(sum(data_set[1].values())+sum(data_set[0].values()))))/((data_set[1][word]+(data_set[0][word] if word in data_set[0] else 0))/(sum(data_set[1].values())+sum(data_set[0].values())))
		else:
			p_word_positive_temp = 0
		if word in data_set[0]:
			p_word_negative_temp = ((data_set[0][word]/sum(data_set[0].values()))*(sum(data_set[0].values())/(sum(data_set[1].values())+sum(data_set[0].values()))))/((data_set[0][word]+(data_set[1][word] if word in data_set[1] else 0)/(sum(data_set[1].values())+sum(data_set[0].values()))))
		else:
			p_word_negative_temp = 0
		p_word_positive += p_word_positive_temp
		p_word_negative += p_word_negative_temp
